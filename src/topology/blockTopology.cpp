...
namespace HSF
{
BlockTopology::BlockTopology() : commcator_(NULL) {}

BlockTopology::BlockTopology(Communicator &other_comm) : commcator_(&other_comm)
{
}
...
{
  if (!this->commcator_) this->commcator_ = &(topo.getCommunicator());
  ...
  for (int i = 0; i < this->face2Cell_.size(); ++i)
  {
    for (int j = this->face2Cell_.startIdx[i];
         j < this->face2Cell_.startIdx[i + 1];
         ++j)
    {
      // ghost单元不进行映射
  ...
  for (int i = 0; i < this->cell2Face_.size(); ++i)
  {
    for (int j = this->cell2Face_.startIdx[i];
         j < this->cell2Face_.startIdx[i + 1];
         ++j)
    {
...
Array<label> BlockTopology::reorderFaceTopo(Topology &topo)
{
  ...
  this->commcator_->gather(
      "gather", &blockNum, sizeof(int), blockNum_mpi, sizeof(int), 0);
  this->commcator_->finishTask("gather");
  ...
  this->commcator_->bcast("bcast", &maxBlockNum, sizeof(int), 0);
  this->commcator_->finishTask("bcast");
  this->commcator_->bcast("bcast1", &irank, sizeof(int), 0);
  this->commcator_->finishTask("bcast1");
  ...
  this->commcator_->bcast("bcast2", buffer, maxBlockNum * sizeof(label), irank);
  this->commcator_->finishTask("bcast2");
  ...
  {
    face2NodeBlkNew.insert(
        face2NodeBlkNew.end(), face2NodeBlk[i].begin(), face2NodeBlk[i].end());
  }
...
Array<label> BlockTopology::reorderCellTopo(Topology &topo)
{
  ...
  this->commcator_->bcast("bcast1", &maxBlockNum, sizeof(int), 0);
  this->commcator_->bcast("bcast2", &irank, sizeof(int), 0);
  this->commcator_->finishTask("bcast1");
  this->commcator_->finishTask("bcast2");
  par_std_out("The maximum block num is %d at rank %d\n", maxBlockNum, irank);
  ...
  this->commcator_->bcast("bcast3", buffer, maxBlockNum * sizeof(label), irank);
  this->commcator_->finishTask("bcast3");
  ...
  {
    cell2CellBlkNew.insert(
        cell2CellBlkNew.end(), cell2CellBlk[i].begin(), cell2CellBlk[i].end());
  }
...
void BlockTopology::reorderOtherTopo(const Array<label> &map,
                                     const ArrayArray<label> &oldTopo,
                                     ArrayArray<label> &newTopo)
{
