...
//--------------------------------------------------------------
  ...
  static inline int createCart(const Communicator& refComm, 
      Communicator& newComm, const int ndims, const int* dims, int* periods, 
      int reorder = 1)
  {
...
//--------------------------------------------------------------
  ...
  {
    return globalManager->isParallel();
  }
  ...
  static inline int getGlobalId()
  {
    return globalManager->getGlobalId();
  }

  static inline int getGlobalSize()
  {
    return globalManager->getGlobalSize();
  }
...
//--------------------------------------------------------------
   ...
   * @return 
   */
  static inline OStream& pOut()
  {
    return globalManager->pOut();
  }
   ...
   * @return 
   */
  static inline OStream & pErr()
  {
    return globalManager->pErr();
  }
...
#define SOUT(id) if( COMM::getGlobalId() == id ) std::cout<<"proc["<<id<<"]: "
