...
//--------------------------------------------------------------
  ...
  static CommunicationManager* New(int* argc, char*** argv, 
      const Word type = "MPI") 
  {
    return CommunicationManager::selectingTable.find(type)->second(argc, 
        argv);
  }
...
//--------------------------------------------------------------
   ...
   * @return 
   */
  virtual int duplicate(const Communicator& refComm, Communicator& newComm );
   ...
   * @return 
   */
  virtual int split(const Communicator& refComm, Communicator& newComm, 
   ...
   * @return 
   */
  virtual int extract(const Communicator& refComm, Communicator& newComm,
   ...
   * @return 
   */
  virtual int reBalance(const Communicator& oldComm, Communicator& newComm,
      const int weight, const float thresholdRatio );
 
  // TODO, Cartesian-topology communicator constructor
  virtual int createCart(const Communicator& refComm, Communicator& newComm, 
      const int ndims, const int* dims, int* periods, int reorder = 1 )
  {
  ...
  virtual int createGraph(const Communicator& refComm, 
      Communicator& newComm, const int degree, const int* dests, 
      const int* weights,  const int reorder = 1 )
  {
    cerr<<__FILE__<<" +"<<__LINE__<<":"<<endl
      <<__FUNCTION__<<":"<<endl
...
//--------------------------------------------------------------
  ...
  {
    if( globalSize_ > 1 ) return true;
    else
      return false;
  }

  virtual Communicator& getGlobalComm(){return *globalComm_; }

  virtual int getGlobalId(){return globalRank_; }

  virtual int getGlobalSize(){return globalSize_; }
   ...
   */
  virtual float calculateBalance(const Communicator& refComm, const int weight);
...
//--------------------------------------------------------------
   ...
   * @return 
   */
  virtual OStream& pOut(){ return *pOut_; }
   ...
   * @return 
   */
  virtual OStream& pErr(){return *pErr_; }
